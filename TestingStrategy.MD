...
| `test_frontend.py` | 211 | Upload handler, multi-page routing, page layouts, action view math, dashboard KPIs/highlights, filter controls (incl. game type toggle), guide page, showdown section (incl. winner + hand description), favourites, DataTable sorting, decimal format helpers, opponent profile cards, profiles panel, villain summary line, first-action archetype badge, session report view (KPI strip, narrative, EV summary, flagged hands, equity-unavailable note), session report navigation (breadcrumb + browse button wiring), settings analysis controls (sample count, lucky/unlucky thresholds, min-hands) |
