...
| `test_frontend.py` | 182 | Upload handler, multi-page routing, page layouts, action view math, dashboard KPIs/highlights, filter controls (incl. game type toggle), guide page, showdown section (incl. winner + hand description), favourites, DataTable sorting, decimal format helpers, opponent profile cards, profiles panel, villain summary line, first-action archetype badge |
| `test_analysis.py` | 115 | Queries and stats: VPIP, PFR, Win Rate, AF, WTSD, timeline, 3-bet (incl. blind-position regression), c-bet, EV, equity cache, date filter, currency filter, session player stats, player archetype classification, session analysis queries |
