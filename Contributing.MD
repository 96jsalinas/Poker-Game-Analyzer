...
* **Formatting & Linting:** [Ruff](https://docs.astral.sh/ruff/)
    ...
    * ⚠️ **Scope rule:** Both commands run project-wide by default and will reformat files you didn't touch. Always stage and commit formatting changes **together** with the feature/fix changes that triggered them — never as a separate commit. If you have unrelated uncommitted changes in your working tree, scope the commands to only the files you changed (e.g. `ruff format src/pokerhero/ingestion/ tests/test_ingestion.py`) to avoid picking up unrelated diffs.
...
│       ├── analysis/
│       ├── database/
│       └── frontend/     # Dash multi-page app (pages/ subpackage)
├── tests/
│   └── fixtures/
├── data/
├── pyproject.toml
...
### 3a. No `Any` — Use `TypedDict` for heterogeneous dicts
...
* **Pre-commit Hooks:** The repository ships a `.pre-commit-config.yaml` that defines `ruff check`, `ruff format`, and `mypy` hooks using the local Python environment. Run `pre-commit install` once after cloning to activate them. On every commit, the hooks run automatically and abort if any check fails.
...
* **Document discoveries immediately:** Any non-obvious finding made during implementation — format quirks, edge cases, data assumptions, design decisions, known limitations — must be recorded in the appropriate `.MD` file in the same commit as the code that handles it. The right file to use:
