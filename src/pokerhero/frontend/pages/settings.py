    ...
    children=[
        ...
        html.Div(
            ...
            children=[
                ...
                html.P(
                    "Set your target percentages for VPIP, PFR, and 3-Bet. "
                    "These are shown alongside your actual stats on the Dashboard.",
                    style={"color": "#555", "fontSize": "14px"},
                ),
                html.Div(
                    ...
                    children=[
                        html.Div(
                            [
                                ...
                                html.P(
                                    "Your ideal Voluntarily Put In Pot percentage.",
                                    style={
                                        "fontSize": "12px",
                                        "color": "#777",
                                        "margin": "0 0 6px 0",
                        ...
                        html.Div(
                            [
                                ...
                                html.P(
                                    "Your ideal Pre-Flop Raise percentage.",
                                    style={
                                        "fontSize": "12px",
                                        "color": "#777",
                                        "margin": "0 0 6px 0",
                        ...
                        html.Div(
                            [
                                ...
                                html.P(
                                    "Your ideal 3-Bet percentage (re-raise vs opener).",
                                    style={
                                        "fontSize": "12px",
                                        "color": "#777",
                                        "margin": "0 0 6px 0",
...
@callback(
    Output("settings-target-vpip", "value"),
    Output("settings-target-pfr", "value"),
    Output("settings-target-3bet", "value"),
    Input("_pages_location", "pathname"),
    prevent_initial_call=False,
)
...
@callback(
    Output("settings-target-vpip-saved", "children"),
    Input("settings-target-vpip", "value"),
    prevent_initial_call=True,
)
def _save_target_vpip(value: int | None) -> str:
    """Persist target_vpip to settings table on change."""
    if value is None:
        return ""
    db_path = _get_db_path()
    if db_path == ":memory:":
    ...
    conn = get_connection(db_path)
    try:
        set_setting(conn, "target_vpip", str(int(value)))
        conn.commit()
    finally:
        conn.close()
    return "✓ saved"
...
@callback(
    Output("settings-target-pfr-saved", "children"),
    Input("settings-target-pfr", "value"),
    prevent_initial_call=True,
)
def _save_target_pfr(value: int | None) -> str:
    """Persist target_pfr to settings table on change."""
    if value is None:
        return ""
    db_path = _get_db_path()
    if db_path == ":memory:":
    ...
    conn = get_connection(db_path)
    try:
        set_setting(conn, "target_pfr", str(int(value)))
        conn.commit()
    finally:
        conn.close()
    return "✓ saved"
...
@callback(
    Output("settings-target-3bet-saved", "children"),
    Input("settings-target-3bet", "value"),
    prevent_initial_call=True,
)
def _save_target_3bet(value: int | None) -> str:
    """Persist target_3bet to settings table on change."""
    if value is None:
        return ""
    db_path = _get_db_path()
    if db_path == ":memory:":
    ...
    conn = get_connection(db_path)
    try:
        set_setting(conn, "target_3bet", str(int(value)))
        conn.commit()
    finally:
        conn.close()
    return "✓ saved"
